...
int main()
{
    ...
    delete_get_mark(study_and_get(creat_get_mark(&get_mark)));
    return 0;
}
...
struct get_mark * study_and_get(struct get_mark * head){
    int count = 1;
    if (head == NULL)
		return 0;
	 current = head;
	while (current != NULL){
		...
		printf("Студент: %s вивчив предмет %s і отримав таку оцінку %d",(*current).student.first_name, (*current).subject.subject_name, (*current).mark);
		printf("\n");
